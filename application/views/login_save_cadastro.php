<div class="form">
    <form id="form" action="<?=site_url()?>/login/doSaveCadastro" method="POST">
        <p class="sub-title">
            <b class="versalete">Olá <?= $login->nome ?>, seja bem-vindo(a)!</b>
        </p>
        <p>Mantenha os seus dados atualizados para que você obtenha o melhor de nosso site.</p>

        <? if($this->auth->checkAdmin(NO_REDIRECT) == true): ?>
            <p>Acesso administrador, <a href="<?= site_url() ?>/tarot">clique aqui.</a></p>
        <? endif; ?>
                ...
                <div class="widget-bottom">
                    <div class="widget-content">
                        <div class="textwidget extrato">
                            <span><input type="text" name="nome" placeholder="Nome completo" value="<?= $login->nome ?>"></span>
                            <span>
                                <input type="text" name="data_nascimento" placeholder="Data de nascimento" value="<?= mysql_to_br($login->data_nascimento);?>">
                                <span>(dd/mm/aaaa)</span>
                            </span>
                            <span>
                                <input type="text" name="profissao" placeholder="Profiss&atilde;o" value="<?= $login->profissao ?>">
                            </span>
                            <span>
                                <? $posicoesArray = (array) $posicoes; ?>
                                <select name="posicao" style="width: 317px; margin: 10px 20px 0 0;">
                                    <option value="0">Qual a sua posi&ccedil;&atilde;o?</option>
                                    <? $posicaoOutros = (!in_array(htmlentities(utf8_decode($login->posicao)), $posicoesArray)) ? true : false ?>
                                    <? foreach($posicoes as $posicao): ?>
                                        <? $selected = (htmlentities(utf8_decode($login->posicao)) == $posicao) ? "selected='selected'" : "" ?>
                                        <? $selected = ($posicaoOutros AND $posicao == "Outros") ? "selected='selected'" : $selected ?>
                                        <option <?= $selected ?>><?= $posicao ?></option>
                                    <? endforeach; ?>
                                </select>
                                <span>
                                    <? $posicaoOutros = (!in_array(htmlentities(utf8_decode($login->posicao)), $posicoesArray)) ? $login->posicao : "" ?>
                                    <input style="width: 200px;" type="text" name="posicao_outros" placeholder="Outros?" value="<?= $posicaoOutros ?>"/>
                                </span>
                            </span>
                        </div>
            ...
            <div class="widget">
                <div class="widget-bottom">
                    <div class="widget-content">
                        <div class="textwidget extrato">
                            <span>
                                <input type="text" name="telefone" placeholder="Telefone" value="(<?= $login->ddi ?>)(<?= $login->ddd ?>) <?=$login->telefone?>">
                                <span>(ddi)(ddd) 9999-9999</span>
                            </span>
                            <span>
                                <input type="text" name="email" placeholder="E-mail" value="<?= $login->email ?>">
                ...
                <div class="widget-bottom">
                    <div class="widget-content">
                        <div class="textwidget extrato">
                            ...
                            <span>
                                <? $estado = ($login->estado_outro != "") ? $login->estado_outro : $login->estado ?>
                                <input type="text" name="estado" placeholder="Estado" value="<?= $estado ?>">
                                <input type="hidden" name="estado_id" value="<?= $login->cod_estado ?>" />
                                <span id="estado-not-found" class="hidden erro-not-found"><b>Erro:</b> Nenhum resultado encontrado</span>
                            </span>
                            <span>
                                <? $cidade = ($login->cidade_outro != "") ? $login->cidade_outro : $login->cidade ?>
                                <input type="text" name="cidade" placeholder="Cidade" value="<?= $cidade ?>">
                                <input type="hidden" name="cidade_id" value="<?= $login->cod_cidade ?>" />
        ...
        <div class="clearfix tMargin30">
            <a id="btnSubmit" class="newGreenButton submit"><span>Atualizar</span></a>
        </div>
    </form>
</div>
...
function checkEmail(){
    ...
    $.ajax({
        ...
        data: { email: email , codUsuario : "<?= Auth::getData('cod') ?>" }
    }).done(function( msg ) {
...
$("document").ready(function(){
    ...
    $("a.submit").click(function(evt){
        ...
        || $("input[name=senha]").val() != $("input[name=senha_confirmar]").val() ){
            ...
            if($("input[name=senha]").val().length > 0){
                valid = false;

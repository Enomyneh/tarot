...
<? foreach($casasPreenchidas as $key => $casa): ?>
    ...
    <div class="<?=$class?>" id="div-casa-<?=$casa["casaCarta"]->cod_casa_carta?>">
        <div class="clearfix tMargin20">
            <a class="newGreenButton" name="casa-anterior" casa="<?=$casa["casaCarta"]->cod_casa_carta?>"><span>Casa anterior</span></a>
            <a class="newGreenButton right" name="proxima-casa" casa="<?=$casa["casaCarta"]->cod_casa_carta?>"><span>Pr&oacute;xima casa</span></a>
        </div>
        ...
        <? if($casa["comprado"] == false): ?>
            <div class="tMargin10">
                Continue lendo a sua interpretação por apenas R$1,00 real a página. Acima você tem uma noção da quantidade de conteúdo observando a área criptografada.
            </div>
            <div style="clear: both; margin-top: 20px;" class="clearfix">
                <? if($logado): ?>
                    <a href="<?=site_url()?>/compra/jogo/confirm" class="newGreenButton">
                        <span>Obter leitura completa</span>
                    </a>
                <? else: ?>
                    <a href="<?=site_url()?>/compra/jogo/confirm_without_login" class="newGreenButton"><span>Obter leitura completa</span></a>
                <? endif; ?>
            </div>
        <? endif; ?>
        ...
        <div class="clearfix" style="text-align: center; margin-top:30px; margin-left: 220px;">
            <a href="<?=site_url()?>/jogo/escolherSetorVida" class="newGreenButton"><span>Come&ccedil;ar um novo jogo</span></a>
        </div >
    </div>
<? endforeach; ?>
<script>
